...
const { produtos, Cupom, Keys, giftcards, drops, usuariosinfo, estatisticas, General } = require("../databases");
...
function produtoscriados(interaction) {

    var u = produtos.fetchAll()
    var blocks = []
...
function criadoscupons(interaction) {

    var u = Cupom.fetchAll()
    var blocks = []
    for (let iiiiii = 0; iiiiii < u.length; iiiiii++) {
...
function criadoskeys(interaction) {
    var u = Keys.fetchAll()
    var blocks = []
    for (let iiiiii = 0; iiiiii < u.length; iiiiii++) {
        const element = u[iiiiii].ID
        const cargo = u[iiiiii].data.cargo

        blocks.push(`${obterEmoji(12)} **| Key:** ${element}\n${obterEmoji(7)} **| ID do cargo:** ${cargo}\n${obterEmoji(13)} **| Cargo:** <@&${cargo}>`)
    }
    const embed = new EmbedBuilder()
        .setTitle(`Erro - Sistema de Vendas`)
        .setDescription(`Não encontrei nada desse tipo cadastrado no bot!`)
...
function criadosgifts(interaction) {
    var u = giftcards.fetchAll()
    var blocks = []
    for (let iiiiii = 0; iiiiii < u.length; iiiiii++) {
        const element = u[iiiiii].ID
...
function criadosdrop(interaction) {
    var u = drops.fetchAll()
    var blocks = []
    for (let iiiiii = 0; iiiiii < u.length; iiiiii++) {
        const element = u[iiiiii].ID
...
function rankprosdutos(interaction) {
...
function criadossemstock(interaction) {
