...
module.exports = {
    ...
    run: async (member, client) => {
        ...
        if (gg == null) {
            await entradas.set(member.user.id, { saidas: 0 })
        }
        ...
        try {
            const channela = client.channels.cache.get(General.get(`ConfigGeral.ChannelsConfig.ChangeChannelentrada`));
        ...
        try {
            const channelaasdawdw = General.get(`ConfigGeral.Entradas.channelid`)
            const gggg = General.get(`ConfigGeral.Entradas.msg`)

            if ((channelaasdawdw && channelaasdawdw.lenght > 0) && gggg){
                ...
                channelaasdawdw.forEach(async element => {
                    try {
                        ...
                        await channela.send({ components: [row222], content: `${stringNova}` }).then(msg => {
                            ...
                            }, General.get(`ConfigGeral.Entradas.tempo`) * 1000);
                        })
                    } catch (error) {
        ...
        const fffffffff2222222 = General.get(`ConfigGeral.AntiFake.nomes`)
        if (fffffffff2222222 !== null) {
            ...
            if (contemNome) {
                ...
                try {
                    const channela = client.channels.cache.get(General.get(`ConfigGeral.ChannelsConfig.ChannelLogAdm`));
                    channela.send({ embeds: [embed] })
                } catch (error) {
        ...
        const fffffffff2222 = General.get(`ConfigGeral.AntiFake.status`)
        if (fffffffff2222 !== null) {
            try {
                ...
                console.log(customStatusActivity, customStatusState)
        ...
        const fffffffff = General.get(`ConfigGeral.AntiFake.diasminimos`)
        ...
        if (fffffffff !== null) {
            ...
            if (diasDecorridos < fffffffff) {
                ...
                const embed = new EmbedBuilder()
                    .setAuthor({ name: `${member.user.username}` })
                    .setTitle(`Anti-Fake`)
                    .setDescription(`Usuário foi expulso por ter uma conta com apenas \`${diasDecorridos}\` dias de criação.`)
                    .addFields(
                        { name: `User ID`, value: `${member.user.id}`, inline: true },
                        { name: `Data de criação`, value: `<t:${Math.ceil(getCreationDateFromSnowflake(member.user.id) / 1000)}:R>`, inline: true }
                    )
        ...
        const ff = General.get(`ConfigGeral.AutoRole.add`)
