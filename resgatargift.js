...
module.exports = {
  ...
  run: async (client, interaction, message) => {
    ...
    var keyverify = await giftcards.get(key)
    if (keyverify == null) return interaction.reply({ content: `${obterEmoji(7)} | você tentou resgatar um gift inexistente`, ephemeral: true })

    PagamentosSaldos.set(`${interaction.user.id}.SaldoAccount`, Number(PagamentosSaldos.get(`${interaction.user.id}.SaldoAccount`)) + Number(keyverify.valor))
    const embed = new EmbedBuilder()
      ...
      .setDescription(`Você acabou de resgatar um gift no valor de: \`R$${Number(keyverify.valor).toFixed(2)}\` , agora você está com \`R$${Number(PagamentosSaldos.get(`${interaction.user.id}.SaldoAccount`)).toFixed(2)}\``)
      .setFooter({ text: `${interaction.guild.name} - Todos os direitos reservados.`, iconURL: `${client.user.displayAvatarURL()}` })
      .setTimestamp()


    interaction.reply({ embeds: [embed], ephemeral: true })
    giftcards.delete(key)

    const log = interaction.guild.channels.cache.get(General.get(`ConfigGeral.ChannelsConfig.ChannelLogAdm`))
    try {
