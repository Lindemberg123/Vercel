...
module.exports = {
  ...
  run: async (client, interaction, message) => {
    ...
    var tt222 = produtos.get(`${prod}_${interaction.guild.id}`)
    var tt2222 = produtos.get(`${prod}_${interaction.guild.id}.settings.estoque`)
    ...
    var finalproduto = ''
    for (var j = 0; j < qtd; j++) {
      var tt = produtos.get(`${prod}_${interaction.guild.id}`)
      var ooooaoo = tt.settings.estoque[0]

      produtos.pull(`${prod}_${interaction.guild.id}.settings.estoque`, (element, index, array) => index == 0)
