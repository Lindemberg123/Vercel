    ...
    run: async (interaction, client) => {
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.startsWith('changeporcentagemcupom')) {
                ...
                .setColor(General.get(`ConfigGeral.ColorEmbed`) == '#635b44' ? 'Random' : `${General.get(`ConfigGeral.ColorEmbed`)}`)
                    ...
                    .setDescription(`**Porcentagem de Desconto Atual:** \`${Number(Cupom.get(`${t.cupom}.porcentagem`)).toFixed(0)}%\`\nEnvie a nova porcentagem abaixo:`)
                ...
                interaction.message.edit({ embeds: [embed], components: [] }).then(msg => {
                    ...
                    collector.on('collect', async (message) => {
                        ...
                        if (apenasNumeros) {
                            ...
                            Cupom.set(`${t.cupom}.porcentagem`, Number(u).toFixed(0))
            ...
            if (interaction.customId.startsWith('changevalorminimocupom')) {
                ...
                .setColor(General.get(`ConfigGeral.ColorEmbed`) == '#635b44' ? 'Random' : `${General.get(`ConfigGeral.ColorEmbed`)}`)
                    .setTitle(`${client.user.username} | Gerenciar Cupom`)
                    .setDescription(`**Valor Mínimo Atual: ** \`R$${Number(Cupom.get(`${t.cupom}.valorminimo`)).toFixed(2)}\`\nEnvie o novo Valor Mínimo abaixo:`)
                ...
                interaction.message.edit({ embeds: [embed], components: [] }).then(msg => {
                    ...
                    collector.on('collect', async (message) => {
                        ...
                        if (apenasNumerosComPonto) {
                            ...
                            msg.reply({ content: `${obterEmoji(8)} | O valor mínimo desse cupom foi atualizado com sucesso para R$ ${Number(u).toFixed(2)}.`, ephemeral: true }).then(msg2 => {
                                setTimeout(() => {
                                    msg2.delete()
                                }, 3000);
                            })
                            Cupom.set(`${t.cupom}.valorminimo`, Number(u).toFixed(2))
                            StartConfigCupom(interaction, client, interaction.user.id, t.cupom)
                        } else {
            ...
            if (interaction.customId.startsWith('changequantidadecupom')) {
                ...
                .setColor(General.get(`ConfigGeral.ColorEmbed`) == '#635b44' ? 'Random' : `${General.get(`ConfigGeral.ColorEmbed`)}`)
                    .setTitle(`${client.user.username} | Gerenciar Cupom`)
                    .setDescription(`**Quantidade Atual:** \`${Number(Cupom.get(`${t.cupom}.quantidade`)).toFixed(0)}\`\nEnvie a nova quantidade abaixo:`)
                ...
                interaction.message.edit({ embeds: [embed], components: [] }).then(msg => {
                    ...
                    collector.on('collect', async (message) => {
                        ...
                        if (apenasNumeros) {
                            ...
                            Cupom.set(`${t.cupom}.quantidade`, Number(u).toFixed(0))
                            StartConfigCupom(interaction, client, interaction.user.id, t.cupom)
                        } else {
                            msg.reply({ content: `${obterEmoji(22)} | "${Number(u).toFixed(0)}" Valor Inválido!`, ephemeral: true }).then(msg2 => {
                                setTimeout(() => {
            ...
            if (interaction.customId.startsWith('yhds8cdy98vby9duv9s7sdb')) {
                ...
                Cupom.delete(`${t.cupom}.categoria`)
                StartConfigCupom(interaction, client, interaction.user.id, t.cupom)
            }
            ...
            if (interaction.customId.startsWith('changecategoriacupom')) {
                ...
                .setColor(General.get(`ConfigGeral.ColorEmbed`) == '#635b44' ? 'Random' : `${General.get(`ConfigGeral.ColorEmbed`)}`)
                    .setTitle(`${client.user.username} | Gerenciar Cupom`)
                    .setDescription(`**Categoria atual:** ${Cupom.get(`${t.cupom}.categoria`) == null ? `\`Este cupom pode ser utilizado em qualquer produto!\`` : `<#${Cupom.get(`${t.cupom}.categoria`)}>`}\nSelecione abaixo nova categoria:`)
            ...
            if (interaction.customId.startsWith('changecargocupom')) {
                ...
                .setColor(General.get(`ConfigGeral.ColorEmbed`) == '#635b44' ? 'Random' : `${General.get(`ConfigGeral.ColorEmbed`)}`)
                    .setTitle(`${client.user.username} | Gerenciar Cupom`)
                    .setDescription(`**Cargo atual:** ${Cupom.get(`${t.cupom}.cargo`) == null ? `\`Este cupom pode ser utilizado por qualquer usuário!\`` : `<@&${Cupom.get(`${t.cupom}.cargo`)}>`}\nSelecione abaixo novo CARGO:`)
        ...
        if (interaction.isChannelSelectMenu()) {
            if (interaction.customId == 'changequantidadecupom') {
                ...
                Cupom.set(`${t.cupom}.categoria`, interaction.values[0])
        ...
        if (interaction.isRoleSelectMenu()) {
            if (interaction.customId == 'changecargocupom') {
                ...
                Cupom.set(`${t.cupom}.cargo`, interaction.values[0])
        ...
        if (interaction.isAutocomplete()) {
            if (interaction.commandName == 'configcupom') {
                ...
                var teste = Cupom.fetchAll()

                var nomeDigitado = interaction.options.getFocused().toLowerCase();
                var produtosFiltrados = Cupom.filter(x => x.ID?.toLowerCase().includes(nomeDigitado));
                var produtosSelecionados = produtosFiltrados.slice(0, 25);

                const config = produtosSelecionados.map(x => {
                    return {
                        name: (`✔ | CUPOM - ${x.ID} 💸 | Desconto - ${x.data.porcentagem} ${obterEmoji(12)} | Quantidade - ${x.data.quantidade}`).slice(0, 99),
                        value: `${x.ID}`
                    }
                })
